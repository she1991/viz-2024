...
def check_flip_property(data):
    ...
    for state_obj in data:
        state_code = state_obj["state"]  # Changed to access state directly
        
        for county in state_obj["counties"]:  # Changed to access counties directly
...
input_path = "../public/data/transformed_states.json"
...
try:
    ...
    # Filter out the special states
    regular_states = [state for state in data if state["state"] not in [
        "Counties Which Flipped",
        "Flipped Once",
        "Flipped Twice",
        "Flipped Thrice"
    ]]
    ...
    # Check for issues
    issues = check_flip_property(regular_states)
